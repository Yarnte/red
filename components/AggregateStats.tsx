...
const AggregateStats: React.FC<AggregateStatsProps> = ({ sites, readings }) => {
    ...
    const CO2_REDUCTION_FACTOR_TONNES_PER_KWH = 0.0007; // Approximate factor for solar PV
    ...
    const stats = useMemo(() => {
        if (!latestMonth) {
            return { totalExpected: 0, totalProduction: 0, lowPerformingCount: 0, co2Reduction: 0, equivalentTrees: 0, totalCapacity: 0 };
        }
        ...
        for (const site of sites) {
             ...
             // Sum capacity for all sites in the view, regardless of whether they have a reading for the latest month
        ...
        <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
                ...
                <StatCard 
                    ...
                    value={stats.lowPerformingCount.toString()}
                    unit="sites"
                    colorClass={stats.lowPerformingCount > 0 ? "bg-red-500" : "bg-yellow-500"}
                />
                 <StatCard
                    icon={<WindIcon className="h-6 w-6 text-white" />}
                    title="CO2 Reductions"
                    value={stats.co2Reduction.toFixed(1)}
                    unit="tonnes"
                    colorClass="bg-gray-500"
                />
                <StatCard
                    icon={<LeafIcon className="h-6 w-6 text-white" />}
                    title="Equivalent Trees Planted"
                    value={Math.round(stats.equivalentTrees).toLocaleString()}
                    unit="trees"
