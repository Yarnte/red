            ...
            <div className="bg-white rounded-lg shadow-2xl p-8 w-full max-w-md m-4">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <select id="site" value={siteId} onChange={(e) => setSiteId(Number(e.target.value))} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 rounded-md">
                    ...
                    <div>
                        ...
                        <input type="month" id="date" value={date} onChange={e => setDate(e.target.value)} max={new Date().toISOString().slice(0, 7)} className="mt-1 block w-full pl-3 pr-3 py-2 text-base border-gray-300 rounded-md" />
                    </div>
                    <div>
                        <label htmlFor="reading" className="block text-sm font-medium text-gray-700">Monthly Production (kWh)</label>
                        <input type="number" id="reading" value={valueKwh} onChange={e => setValueKwh(e.target.value)} placeholder="Export Counter Reading (2.8.0 / EXP)" className="mt-1 block w-full pl-3 pr-3 py-2 text-base border-gray-300 rounded-md" />
                    </div>
                    ...
                    <div className="flex justify-end space-x-3 pt-4">
                        ...
                        <button type="submit" disabled={isDuplicate} className="px-4 py-2 bg-brand-blue text-white rounded-md hover:bg-brand-blue-light transition disabled:bg-gray-400">Save Reading</button>
                    </div>
                </form>
            </div>
        </div>
