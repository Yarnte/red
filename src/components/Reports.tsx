...
const Reports: React.FC<ReportsProps> = ({ sites, readings }) => {
    ...
    const generateCSV = () => {
        ...
        for (const reading of filteredReadings) {
            ...
            if (site) {
                const efficiency = site.expectedMonthlyKwh > 0 ? ((reading.valueKwh / site.expectedMonthlyKwh) * 100).toFixed(2) : '0.00';
                const row = [`"${site.name.replace(/"/g, '""')}"`, reading.date, reading.valueKwh, site.expectedMonthlyKwh, efficiency].join(',');
                csvRows.push(row);
            }
        }
    ...
    return (
        <div className="bg-white p-8 rounded-xl shadow-lg">
            ...
            <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4">
                <select value={reportType} onChange={e => setReportType(e.target.value as 'monthly' | 'yearly')} className="p-2 border border-gray-300 rounded-md">
                ...
                {reportType === 'monthly' ? (
                    ...
                    <input type="number" value={selectedYear} onChange={e => setSelectedYear(parseInt(e.target.value, 10))} min="2020" max={new Date().getFullYear()} className="p-2 border border-gray-300 rounded-md" />
                )}
                <button onClick={generateCSV} className="flex items-center space-x-2 w-full sm:w-auto justify-center px-6 py-2 bg-brand-yellow text-brand-blue font-bold rounded-md shadow">
