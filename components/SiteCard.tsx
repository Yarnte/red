...
const SiteCard: React.FC<SiteCardProps> = ({ site, latestReading, onClick }) => {
    ...
    const efficiency = hasReading ? (latestReading.valueKwh / site.expectedMonthlyKwh) * 100 : 0;

    let borderColorClass = 'border-brand-blue';
    let efficiencyColor = 'text-gray-800'; // Default for no reading
    let alertIcon = null;
    ...
    if (hasReading) {
        if (efficiency <= 50) {
            ...
            alertIcon = (
                <div className="flex items-center space-x-1 text-red-500 bg-red-100 px-2 py-1 rounded-full">
            ...
            <div className={`p-4 border-l-8 ${borderColorClass}`}>
                ...
                <div className="mt-4 grid grid-cols-2 gap-4 text-center">
                    <div className="p-2 bg-gray-50 rounded-lg">
                        ...
                        <p className="text-lg font-semibold text-gray-800 flex items-center justify-center space-x-1">
                          <ZapIcon className="h-5 w-5 text-green-500" />
                          <span>{latestReading ? `${latestReading.valueKwh.toLocaleString()} kWh` : 'N/A'}</span>
                        </p>
                    </div>
                    <div className="p-2 bg-gray-50 rounded-lg">
                        <p className="text-sm text-gray-500">Efficiency</p>
                        <p className={`text-lg font-semibold ${efficiencyColor} flex items-center justify-center space-x-1`}>
                          <SunIcon className="h-5 w-5" />
                          <span>{latestReading ? efficiency.toFixed(1) + '%' : 'N/A'}</span>
                        </p>
                    </div>
                </div>
                <p className="text-xs text-gray-400 mt-3 text-right">
                    Latest Reading: {formatMonth(latestReading?.date)}
                </p>
            </div>
        </div>
    );
